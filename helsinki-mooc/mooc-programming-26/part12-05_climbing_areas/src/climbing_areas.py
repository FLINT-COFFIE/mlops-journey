...
#functions
def sort_by_number_of_routes(climbing_area):
    def no_of_routes(area):
        return area.routes()
    return sorted(climbing_area, key=no_of_routes)

def sort_by_most_difficult(climbing_area):
    def by_difficulty(route):
            hardest = route.hardest_route()
            return hardest.grade
    return sorted(climbing_area, key=by_difficulty, reverse=True)
